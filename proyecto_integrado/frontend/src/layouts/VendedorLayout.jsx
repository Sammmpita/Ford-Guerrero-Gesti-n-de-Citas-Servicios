...
/** Avatar circular: foto de perfil si existe, iniciales si no. */
function ProfileAvatar({ user, size = 'md' }) {
  const foto = user?.vendedor_perfil?.foto
  ...
  if (foto) {
    return (
      <img
        ...
        className={`${dim} rounded-full object-cover ring-1 ring-zinc-700 flex-shrink-0`}
      />
    )
  }
...
const NAV_ITEMS = [
  { label: 'Dashboard',        to: '/vendedor' },
  { label: 'Mis Citas',        to: '/vendedor/citas' },
  { label: 'Mi Disponibilidad', to: '/vendedor/disponibilidad' },
  { label: 'Mi Perfil',        to: '/vendedor/perfil' },
]
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="h-14 bg-white border-b border-gray-100 flex items-center px-8 flex-shrink-0">
          ...
          <div className="ml-auto flex items-center gap-4">
            {user.vendedor_perfil?.especialidad && (
              <span className="text-xs text-gray-400 font-light tracking-wide hidden sm:block">
                {user.vendedor_perfil.especialidad}
              </span>
            )}
